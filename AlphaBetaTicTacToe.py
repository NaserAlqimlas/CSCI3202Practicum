...
def newStateGenerator(state, move, upNext):
...
def minimizer(state, alpha, beta, possibleSpaces, upNext):
    if possibleSpaces == 0 or terminate(state):  # Checking if game over.
        score = winCondition(state)
        return (score, None)

    bestScore = infinity
    bestMove = None
    for move in possibleMoves(state):
        newState = newStateGenerator(state, move, upNext)
        newScore = min(bestScore, maximizer(newState, alpha, beta, possibleSpaces, -upNext)[0])
        if newScore != bestScore:
            bestScore = newScore
            bestMove = move

        if bestScore <= alpha:			#where the pruning magic happens
...
def maximizer(state, alpha, beta, possibleSpaces, upNext):
    if possibleSpaces == 0 or terminate(state):  #Checking if game over.
        score = winCondition(state)
        return (score, None)
    ...
    for move in possibleMoves(state):
        newState = newStateGenerator(state, move, upNext)
        newScore = max(bestScore, minimizer(newState, alpha, beta, possibleSpaces, -upNext)[0])
        if newScore != bestScore:
            bestScore = newScore
            bestMove = move

        if bestScore >= beta:				#where the pruning magic happens
            return (bestScore, move)
...
def AlphaBetaPruning(state, possibleSpaces, upNext):
    alpha = -infinity
    beta = infinity

    if upNext == COMPUTER:
        (bestScore, bestMove) = maximizer(state, alpha, beta, possibleSpaces, upNext)
    else:
        (bestScore, bestMove) = minimizer(state, alpha, beta, possibleSpaces, upNext)
...
def computerTurn(computerMove, playerMove):
    ...
    if possibleSpaces == 9:
        ...
        move = AlphaBetaPruning(gameBoard, possibleSpaces, COMPUTER)
        x, y = move[0], move[1]
